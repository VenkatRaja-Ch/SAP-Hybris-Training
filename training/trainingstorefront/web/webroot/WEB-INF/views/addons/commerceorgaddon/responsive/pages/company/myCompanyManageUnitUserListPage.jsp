...
<c:set var="searchUrl" value="${baseUrl}/${ycommerce:encodeUrl(action)}?unit=${ycommerce:encodeUrl(param.unit)}&role=${ycommerce:encodeUrl(param.role)}&sort=${ycommerce:encodeUrl(searchPageData.pagination.sort)}"/>
...
<template:page pageTitle="${pageTitle}">
                            ...
                            <div class="row">
                                <c:forEach items="${searchPageData.results}" var="user">
                                    <spring:url value="/my-company/organization-management/manage-users/details" var="viewUrl" htmlEscape="false">
                                        <c:choose>
                                            <c:when test="${not empty user.customerId}">
                                                <spring:param name="user" value="${user.customerId}"/>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <div class="col-xs-12 col-sm-6 col-md-4 card ${user.selected ? 'selected' : ''}" id="row-${user.normalizedUid}">
                                        <ul class="pull-left">
                                            <li>
	...
	<script id="enableDisableLinksTemplate" type="text/x-jquery-tmpl">
        {{if selected}}
            <a href="#" url="${fn:escapeXml(membersActionLink)}/deselect/?user={{= encodeURIComponent(id)}}&amp;role=${fn:escapeXml(ycommerce:encodeUrl(param.role))}" class="action-links js-deselectUser"><span class="glyphicon glyphicon-ok"></span></a>
        {{else}}
            <a href="#" url="${fn:escapeXml(membersActionLink)}/select/?user={{= encodeURIComponent(id)}}&amp;role=${fn:escapeXml(ycommerce:encodeUrl(param.role))}" class="action-links js-selectUser"><span class="glyphicon glyphicon-ok"></span></a>
        {{/if}}
    </script>
