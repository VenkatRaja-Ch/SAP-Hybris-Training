...
public class OrderApprovalController extends AbstractSearchPageController
{
			...
			@RequestParam(value = "sort", required = false) final String sortCode, final Model model) throws CMSItemNotFoundException
	{
		...
		final ContentPageModel orderApprovalDashboardPage = getContentPageForLabelOrId(ORDER_APPROVAL_DASHBOARD_CMS_PAGE);
		storeCmsPageInModel(model, orderApprovalDashboardPage);
		setUpMetaDataForContentPage(model, orderApprovalDashboardPage);
			...
			throws CMSItemNotFoundException
	{
		...
		final ContentPageModel orderApprovalDetailPage = getContentPageForLabelOrId(ORDER_APPROVAL_DETAIL_CMS_PAGE);
		storeCmsPageInModel(model, orderApprovalDetailPage);
		setUpMetaDataForContentPage(model, orderApprovalDetailPage);
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}
